...
const getAllPlaceTags = asyncHandler(async (req, res) => {
  const tags = await PlaceTag.find().sort({ placeType: 1, tag: 1 });
  res.status(200).json(tags);
});

//@desc Get tags by place type
//@route GET /place-tags/by-place-type/:placeType
//@access public
const getTagsByPlaceType = asyncHandler(async (req, res) => {
  const tags = await PlaceTag.find({ placeType: req.params.placeType }).sort({ tag: 1 });
  res.status(200).json(tags);
});
...
const createPlaceTag = asyncHandler(async (req, res) => {
  ...
  const existingTag = await PlaceTag.findOne({ placeType, tag });
  if (existingTag) {
    res.status(400);
...
const updatePlaceTag = asyncHandler(async (req, res) => {
  ...
  checkNotFound(placeTag)(req, res, async () => {
    ...
    const oldPlaceType = placeTag.placeType;
    ...
    if (oldTag !== placeTag.tag && oldPlaceType === placeTag.placeType) {
...
const deletePlaceTag = asyncHandler(async (req, res) => {
  ...
  checkNotFound(placeTag)(req, res, async () => {
    ...
    await Place.updateMany(
      { type: placeTag.placeType, tags: placeTag.tag },
